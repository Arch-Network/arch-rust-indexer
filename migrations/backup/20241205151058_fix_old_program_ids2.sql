...
WITH program_mappings AS (
    SELECT DISTINCT
        ...
        CASE 
            ...
            -- If it looks like base58, try to convert it
            WHEN tp.program_id ~ '^[1-9A-HJ-NP-Za-km-z]{32,44}$' THEN 
                encode(decode_base58(tp.program_id), 'hex')
            ELSE NULL
        END as new_id
